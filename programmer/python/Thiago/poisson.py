...
def get_team(df, prompt):
    while True:
        try:
            name = input(prompt)
            team = df.loc[df['name'].str.contains(name, case=False)]

            if len(team) == 1:
                return team
            elif len(team) > 1:
                print('Mais de um time encontrado. Selecione o time correto:')
                print(team[['name', 'league']])
...
def calcular():
    ...
    foa = time_A['off'].values[0]
    fda = time_A['def'].values[0]
    fob = time_B['off'].values[0]
    fdb = time_B['def'].values[0]
    ...
    gols = pd.DataFrame([linha, coluna, under, over],
                        index=[time_A.name.values[0], time_B.name.values[0], 'under', 'over']).T
