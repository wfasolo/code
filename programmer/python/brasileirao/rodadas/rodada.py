...
def rodar(i, df3, df4):
    ...
    gol_A = off_A * def_B * 1.15
    gol_B = off_B * def_A * 0.85

    # correção pelo media de vitórias
    gol_A = gol_A * 10 ** ((s_v_A) / 400)
    gol_B = gol_B * 10 ** ((s_v_B) / 400)

    resul_A = np.random.poisson(off_A, 5)
    resul_A = [x for x in resul_A if x <= 4]

    probab_A = np.around(poisson.pmf(k=resul_A, mu=gol_A) * 100, decimals=1)
    ...
    probab_B = np.around(poisson.pmf(k=resul_B, mu=gol_B) * 100, decimals=1)
